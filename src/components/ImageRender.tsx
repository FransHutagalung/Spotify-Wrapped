...
const ImageRender = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const responseTopTracks = await fetch(`https://api.spotify.com/v1/me/top/tracks?time_range=${term}&limit=5`, {
                ...
                const responseTopArtists = await fetch(`https://api.spotify.com/v1/me/top/artists?time_range=${term}&limit=5`, {
                    headers: { Authorization: `Bearer ${accessToken}` },
                });
    ...
    return loading ? (
        <div className='flex flex-col justify-center h-screen items-center p-4'>
            <p className='text-xl '>Loading...</p>
        </div>
    ) :
        (
            <div className='flex flex-col lg:flex-row-reverse mx-3 lg:p-4 lg:justify-end p-4 lg:gap-[200px] '>
                <div className="lg:w-1/2 w-full flex flex-col mb-3 items-start justify-start gap-2 rounded-md bg-gray-200  p-2">
                    <p className='text-2xl' style={{ fontFamily: 'monospace' }}>Lets Customise Yours</p>
                    <div className="h-8"></div>
                    <div className="flex flex-col w-[96%] mx-auto border border-blue-300 px-2 py-3">
                        <p>Pick Your Term</p>
                        <div className="h-6"></div>
                        <div className="flex gap-1">
                            <button onClick={() => changeTerm('short_term')} className={`border border-black ${term === 'short_term' ? 'border-1 border-black bg-blue-200' : 'bg-white-200'}  rounded-lg px-3 py-1`}>Short Term</button>
                            <button onClick={() => changeTerm('long_term')} className={`border border-black  ${term === 'long_term' ? 'border-1 border-black bg-blue-200' : 'bg-white-200'}  rounded-lg px-3 py-1`}>Long Term</button>
                        </div>
                    </div>


                    <div
  className="flex flex-col w-[96%] mx-auto border border-blue-300 px-2 py-3 pb-4 overflow-x-scroll scrollbar-thin scroll-container-wrapper"
>
  ...
  <div className="flex gap-[10px] whitespace-nowrap ">
    {color.map((e, index) => (
      <div key={index} className="shrink-0">
        <ColorCircle color={e} isUsed={false} />
      </div>
    ))}
  </div>
</div>
                ...
                '
                    >
                            ...
                            <div className="w-full mt-2  h-32 flex justify-center items-end gap-2 ">
                                <div className="h-[100px] flex flex-col justify-end items-center gap-2">
                                    ...
                                    <div className="h-[80px] w-[80px] mb-2  rounded-md">
                                        <img className='rounded-lg object-cover border  w-full h-[100%]'
                                ...
                                <div className="h-[100px] flex flex-col justify-end items-center gap-2">
                                    ...
                                    <div className="h-[110px] w-[110px] mb-2 bg-blue-300 rounded-lg">
                                        <img className='object-cover rounded-md border  w-full h-[100%]'
                                ...
                                <div className="h-[100px] flex flex-col justify-end items-center gap-2">
                                    ...
                                    <div className="h-[80px] w-[80px] mb-2  rounded-md">
                                        ...
                                        <img className='object-cover rounded-lg w-full border h-[100%]'
                            ...
                            <div className="h-full w-1/5 rounded-lg flex flex-col items-center  justify-between bg-red-700 pb-4 ">
                                <div className="flex flex-col items-center">
                                    <div className="w-14 h-32 mx-auto  ">
                                        <img className='object-fit opacity-83' src="/headset2.png" alt="" />
                                    </div>
                                    <div className="mt-[0px]">
                                        {bestSongs.slice(0, 3).map((e, index) => (
                                            <div key={index} className=' h-12 w-12 my-3'>
                                    ...
                                    <div className="px-2">
                                        {bestSongs.map((e, index) => (
                                            <div key={index} className="flex flex-col  p-1 text-black">
                                                <p style={{ fontFamily: 'Montserrat', fontWeight: 'bold', fontSize: '13px' }}> {index + 1}. {e.name.slice(0, 20)} {e.name.length > 20 ? '...' : ''} </p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className=" mb-2 px-3 mt-1 h-[10%] w-full rounded-md  text-white flex justify-center  items-center relative ">
                            <div className="absolute w-full h-full">
                                <img className='object-cover z-0  w-full h-full rounded-xl' src={selectedBgImage ?? ''} alt="" />
                            </div>
